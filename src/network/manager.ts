...
export type PeerEvent =
	...
	| { type: "host-disconnected"; hostNodeId: string }
	| {
			type: "new-leader";
			newHostId: string;
...
function _createPeerListMessage(
...
function createNewLeaderMessage(
	nodeId: string,
	nickname: string,
): ProtocolMessage {
	return create(ProtocolMessageSchema, {
		payload: {
			case: "newLeader",
			value: create(NewLeaderMessageSchema, { nodeId, nickname }),
		},
	});
}
...
export class PeerManager {
	...
	async create(): Promise<string> {
		...
		this._roomId = this._nodeId; // Creator is the initial host
		this._isHost = true;
	...
	async join(remoteNodeId: string): Promise<void> {
		...
		this._nodeId = this.endpoint.nodeId();
		this._roomId = remoteNodeId; // The room host is the peer we're joining
		this._isHost = false;
		...
		const timeoutPromise = new Promise<never>((_, reject) => {
			setTimeout(() => {
				reject(
					new Error(
						"Connection timed out. The room may not exist or is unreachable.",
					),
				);
		...
		switch (message.payload.case) {
			...
			case "peerList": {
				...
				for (const peer of peerListMsg.peers) {
					...
					// If we already have this peer, skip
					if (this.peerInfo.has(peer.nodeId)) continue;
			...
			case "newLeader": {
				const newLeaderMsg = message.payload.value;
				const newHostId = newLeaderMsg.nodeId;

				// If we've already elected this leader (from our own computation), skip
				if (this._roomId === newHostId) {
					break;
				}

				const newHostNickname = newLeaderMsg.nickname;
				const isMe = newHostId === this._nodeId;

				// Update internal state
				this._roomId = newHostId;
				this._isHost = isMe;

				// Emit new-leader event
				this.emit({ type: "new-leader", newHostId, newHostNickname, isMe });
				break;
			}
		}
	}
	...
	private handleDisconnect(nodeId: string): void {
		if (this.peerInfo.has(nodeId)) {
			...
			if (nodeId === this._roomId && !this._isHost) {
				this.electNewLeader();
			}
		}
	}
	...
	private electNewLeader(): void {
		// Emit host-disconnected event to notify UI
		this.emit({ type: "host-disconnected", hostNodeId: this._roomId });

		// Get all connected peers including self
		const candidates = [this._nodeId, ...this.connections.keys()];

		// Sort lexicographically - lowest nodeId wins
		candidates.sort();

		// There should always be at least one candidate (self)
		const newHostId = candidates[0] as string;
		const isMe = newHostId === this._nodeId;

		// Get nickname of new host
		const newHostNickname = isMe
			? this._nickname
			: (this.peerInfo.get(newHostId)?.nickname ?? "unknown");
		...
		// If we're the new host, broadcast the new leader message to all peers
		if (isMe) {
			this.broadcast(createNewLeaderMessage(newHostId, newHostNickname)).catch(
				(err) => {
					this.emit({ type: "error", error: err });
				},
			);
